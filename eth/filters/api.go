...
// enters the transaction pool and was signed from one of the transactions this nodes manages.
func (api *FilterAPI) NewPendingFullTransactions(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	notifier, supported := rpc.NotifierFromContext(ctx)
	if !supported {
		return &rpc.Subscription{}, rpc.ErrNotificationsUnsupported
	}
	...
	go func() {
		...
		for {
			...
			case txns := <-pendingTxs:
				...
				for _, tx := range txns {
					if tx != nil {
						t := ethapi.NewRPCPendingTransaction(tx, api.sys.backend.CurrentHeader(), nil, api.sys.backend.ChainConfig())
